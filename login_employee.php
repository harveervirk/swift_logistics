...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($email_err) && empty($password_err)){
        ...
        $sql = "SELECT email, password,er.Is_Manager FROM employee e
        INNER JOIN employee_role er ON e.Job_Title=er.Job_Title
        WHERE email = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $email, $hashed_password,$manager);
